...
int main(int argc, char **argv) {
    ...
    static struct option longopts[] = {{"config-file", required_argument, 0, 'c'},
        ...
        if (FD_ISSET(idata.fd, &wset)) {
            ...
            if (idata.pending_write_f) {
                ...
                if ((nw = pcap_inject(sfd, idata.pending_write_data, idata.pending_write_size)) < 0) {
                    syslog(LOG_ERR, "Error injecting packet in main loop: pcap_inject(): %s", pcap_geterr(sfd));
                ...
                if (nw != idata.pending_write_size) {
                    if (verbose_f)
                        syslog(LOG_ERR,
                               "pcap_inject(): only wrote %lu bytes "
                               "(rather than %lu bytes)\n",
                               (LUI)nw, (LUI)(idata.pending_write_size));
                ...
                if (pkt_icmp6->icmp6_type == ND_NEIGHBOR_SOLICIT) {
                    ...
                    if (is_ip6_in_address_list(&(idata.ip6_global), &(pkt_ns->nd_ns_target)) ||
                        ...
                        if (hlist.ncandidates >= hlist.maxcandidates) {
                            syslog(LOG_INFO,
                                   "Couldn't add candidate address: Reached limit (%u)"
                    ...
                    if (valid_icmp6_response(&idata, &hlist, pkthdr, pktdata)) {
                        ...
                        if (hptr != NULL) {
                            ...
                            if ((result = create_candidate_andress(&idata, &hlist, hptr)) == -1) {
                                ...
                                syslog(LOG_INFO,
                                       "Could not add candidate address: Reached limit of "
                                       "%u candidate addresses",
                                       hlist.maxcandidates);
                                continue;
                            }
                        }
 ...
 */
void usage(void) { puts("usage: ipv6mon [-c CONFIG_FILE] [-q] [-v] [-h]"); }
        ...
        while ((p + *(p + 1) * 8) <= pkt_end && *(p + 1) != 0 && !error_f) {
            ...
            case ND_OPT_PREFIX_INFORMATION:
                ...
                if ((idata->prefix_ol.nprefix) < idata->prefix_ol.maxprefix) {
                    if ((pio->nd_opt_pi_flags_reserved & ND_OPT_PI_FLAG_ONLINK) && (pio->nd_opt_pi_prefix_len <= 128) &&
                        !is_ip6_in_prefix_list(&(pio->nd_opt_pi_prefix), &(idata->prefix_ol))) {

                        if ((idata->prefix_ol.prefix[idata->prefix_ol.nprefix] = malloc(sizeof(struct prefix_entry))) ==
                            NULL) {
                            if (verbose_f)
                ...
                if (idata->prefix_ac.nprefix < idata->prefix_ac.maxprefix) {
                    if ((pio->nd_opt_pi_flags_reserved & ND_OPT_PI_FLAG_AUTO) && (pio->nd_opt_pi_prefix_len == 64) &&
                        ...
                        if ((idata->prefix_ac.prefix[idata->prefix_ac.nprefix] = malloc(sizeof(struct prefix_entry))) ==
                            NULL) {
                            if (verbose_f)
                        ...
                        (idata->prefix_ac.prefix[idata->prefix_ac.nprefix])->len = pio->nd_opt_pi_prefix_len;
            ...
            case ND_OPT_PREFIX_INFORMATION:
                ...
                if ((pio->nd_opt_pi_flags_reserved & ND_OPT_PI_FLAG_AUTO) && (pio->nd_opt_pi_prefix_len == 64)) {
                    ...
                    for (i = idata->ip6_global_nconfig; i < idata->ip6_global.nprefix; i++) {
                        for (j = 0; j < 4; j++)
                            if (pio->nd_opt_pi_prefix.s6_addr16[j] != (idata->ip6_global.prefix[i])->ip6.s6_addr16[j])
                                break;
    ...
    if ((dst->s6_addr16[0] & htons(0xffc0)) == htons(0xfe80)) {
        ...
        for (i = 0; i < idata->ip6_global.nprefix; i++) {
            ...
            if ((j == full16) && rest16) {
                ...
                if ((dst->s6_addr16[full16] & mask16) ==
                    ((idata->ip6_global.prefix[i])->ip6.s6_addr16[full16] & mask16))
...
int send_neighbor_advert(struct iface_data *idata, pcap_t *pfd, const u_char *pktdata) {
    ...
    if (FD_ISSET(idata->fd, idata->wset)) {
        ...
        if (nw != (ptr - wbuffer)) {
            if (verbose_f)
                syslog(LOG_ERR,
                       "send_neighbor_advert(): pcap_inject(): only wrote %lu bytes "
                       "(rather than %lu bytes)",
    ...
    *prev_nh = IPPROTO_ICMPV6;
    ...
    if (FD_ISSET(idata->fd, idata->wset)) {
        ...
        if (nw != (ptr - wbuffer)) {
            if (verbose_f)
                syslog(LOG_ERR,
                       "send_router_solicit(): pcap_inject(): only wrote %lu bytes "
                       "(rather than %lu bytes)",
                       (LUI)nw, (LUI)(ptr - wbuffer));
...
int valid_icmp6_response(struct iface_data *idata, struct host_list *hlist, struct pcap_pkthdr *pkthdr,
    ...
    case ICMP6_PARAM_PROB:
        ...
        /*
           ...
           header and the payload we included in the ICMPv6 Echo Request
         */
        if ((pkt_end - (unsigned char *)pkt_icmp6) < (sizeof(struct icmp6_hdr) + +sizeof(struct ip6_hdr) +
                                                      sizeof(struct icmp6_hdr) + ICMPV6_ECHO_PAYLOAD_SIZE)) {
            return 0;
        }
    ...
    for (ptr = ifptr; ptr != NULL; ptr = ptr->ifa_next) {
        if (ptr->ifa_addr != NULL) {
            ...
            if (!(idata->ether_flag) && ((ptr->ifa_addr)->sa_family == AF_LINK)) {
                ...
                if (!(idata->ip6_local_flag) &&
                    (((sockin6ptr->sin6_addr).s6_addr16[0] & htons(0xffc0)) == htons(0xfe80))) {
                    if (strncmp(idata->iface, ptr->ifa_name, IFACE_LENGTH - 1) == 0) {
            ...
            if (ptr->ifa_addr != NULL) {
                if ((ptr->ifa_addr)->sa_family == AF_INET6) {
                    ...
                    if ((((sockin6ptr->sin6_addr).s6_addr16[0] & htons(0xffc0)) != htons(0xfe80))) {
                        if (strncmp(idata->iface, ptr->ifa_name, IFACE_LENGTH - 1) == 0) {
                            if ((prefptr = lookup_ip6_in_address_list(&(idata->ip6_global),
                                                                      &(sockin6ptr->sin6_addr))) != NULL) {
...
u_int16_t key(struct host_list *hlist, struct in6_addr *ipv6) {
    return (((hlist->key_l ^ ipv6->s6_addr16[0] ^ ipv6->s6_addr16[7]) ^
             (hlist->key_h ^ ipv6->s6_addr16[1] ^ ipv6->s6_addr16[6])) %
            MAX_LIST_ENTRIES);
}
        ...
        while (chentry != NULL) {
            ...
            /* Check whether the host should be probed */
                ...
                if ((curtime - chentry->lprobed) >= (unicastprobeint + chentry->ffactor)) {
                    if (send_host_probe(pfd, idata, ((chentry->nprobes % 2) ? PROBE_ICMP6_ECHO : PROBE_UNREC_OPT),
                                        chentry) == -1) {
                        if (verbose_f)
...
int send_host_probe(pcap_t *pfd, struct iface_data *idata, unsigned char type, struct host_entry *host) {
    ...
    if (nw != (ptr - (unsigned char *)ether)) {
        if (verbose_f)
            syslog(LOG_ERR,
                   "send_host_probe(): pcap_inject(): only wrote %lu bytes "
                   "(rather than %lu bytes)",
...
int send_multicast_packet(pcap_t *pfd, struct iface_data *idata, struct in6_addr *srcaddr, unsigned char type,
    ...
    if (nw != (ptr - (unsigned char *)ether)) {
        if (verbose_f)
            syslog(LOG_ERR,
                   "send_multicast_packet(): pcap_inject(): only wrote %lu bytes "
                   "(rather than %lu bytes)\n",
                   (LUI)nw, (LUI)(ptr - (unsigned char *)ether));
...
int send_multicast_probe(pcap_t *pfd, struct iface_data *idata, struct host_list *hlist, unsigned char probetype) {
    ...
    if ((*state == SCAN_GLOBAL) && (*naddr < idata->ip6_global.nprefix)) {
        if (send_multicast_packet(pfd, idata, &((idata->ip6_global.prefix[*naddr])->ip6), probetype,
                                  &all_nodes_onlink) != 0) {
            if (verbose_f)
                ...
                if ((logfile = malloc(strlen(value) + 1)) == NULL) {
                    if (verbose_f) {
                        if (showconfig_f)
                            ...
                            syslog(LOG_ERR,
                                   "process_config_file(): Error while allocating memory to store log file path");
                    }
                ...
                if ((lockfile = malloc(strlen(value) + 1)) == NULL) {
                    if (verbose_f) {
                        if (showconfig_f)
                            ...
                            syslog(LOG_ERR,
                                   "process_config_file(): Error while allocating memory to store lock file path");
                    }
...
void sigterm(int signo) { shutdown_f = 1; }
